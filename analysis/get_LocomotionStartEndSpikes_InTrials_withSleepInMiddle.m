...
for i=2:length(pLab(:,1))
    ...
    auxVelocity = (dist*factor)/dtPosition;     % Converting to cm/s
...
for k=2:length(allTimeCueOn+1)
    if abs(allTimeCueOn(k-1) - allTimeCueOn(k)) > 150 % This number is in seconds and is used here to eliminate sleep period during the trials session
...
%%
velocityAndBinPosition = vertcat(velocityAndBinPosition_beforeSleep,velocityAndBinPosition_afterSleep);
%%
timeInfoSession = 1;
lag = 3;
overlap = 0.9;
...
tam = length(I); % 273 for 7s analyses
soma = zeros(tam, 1);
count = 0;
...
count
mediaEndMovment = soma / length(windowsEnd);

plot(mediaEndMovment,'r','lineW',5)
plot([0 300], [5 5], 'k--','linew',2)
plot([78 78], [0 50], 'k--','linew',2)
ylim([0 50])
xlabel('Time (s)');
...
if ~isempty(mediaEndMovment)
    ...
    save(saveName, 'mediaEndMovment', '-v7.3');
    disp('saved')
end
cell2mat(nameRat)
